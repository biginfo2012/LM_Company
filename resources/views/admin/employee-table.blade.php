<table class="table table-separate table-head-custom table-checkable" id="table">
    ...
    @foreach($data as $index => $item)
        <tr style="{{$item->status == 0 ? 'color: red' : ''}}">
            ...
            <td class="p-0 border text-left align-middle px-1">{{$item['department']}}</td>
            ...
            <td class="p-0 border text-center align-middle">
                ...
                <a href="{{route('company.employee-edit', $item['id'])}}" class="btn btn-outline-dark waves-effect ex_change" style="padding: 8px; margin: 5px;">{{__('edit')}}</a>
            </td>
        </tr>
    @endforeach
    </tbody>
